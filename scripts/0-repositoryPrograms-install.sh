...
# Remover repositórios que estarão repetidos
sudo apt-add-repository "deb http://linux.teamviewer.com/deb stable main" --remove
sudo sed -i -e 's/deb \[arch\=amd64] http:\/\/dl.google.com\/linux\/chrome\/deb\/ stable main//' /etc/apt/sources.list.d/google-chrome.list
