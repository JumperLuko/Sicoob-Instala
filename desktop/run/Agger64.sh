...
dotnet452Instalado="$HOME/.wine/drive_c/windows/Microsoft.NET/Framework/v4.0.30319/Microsoft.Windows.ApplicationServer.Applications.45.man"
dotnet452PTBRInstalado="$HOME/.wine/drive_c/windows/Microsoft.NET/Framework/v4.0.30319/pt-BR/aspnet.mfl"
...
if [ -e "$instaladorPasta"/"$dotNet452_PTB" ] && ! [ -e "$dotnet452Instalado" ]; then
    ...
    if [ $? = 0 ]; then
        wineboot 2>> $logFile
        winetricks dotnet452 2>> $logFile
        winetricks win7
        wine "$instaladorPasta"/"$dotNet452_PTB" 2>> $logFile
        wine /opt/sicoob/instaladores/Instalador-agger.exe
    fi
    ...
    wine $HOME/.wine32/drive_c/AggerSeguros/Aggilizador.Application.exe
fi
